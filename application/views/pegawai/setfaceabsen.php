...
<div class="content-wrapper">
    <div class="card">
        ...
        <ul class="list-group list-group-flush">
                    ...
                    <?php
                        for ($i=0; $i<count($all_files); $i++)
                        {
                            ...
                            if (in_array($ext, $supported_format)){
                                ...
                                echo '<img src="'.base_url($image_name) .'" width="100%" alt="'.$image_name.'" class="img-thumbnail" />';
            ...
            <li class="list-group-item">
                <button class="btn btn-primary btn-submit" id="btnSubmit" <?= count($all_files) ? '' : 'style="display:none"' ?> onclick="train()">Simpan & Perbaharui</button>
            </li>
        </ul>
...
async function train()
{
	...
	var request = await fetch('/pegawai/savefaceabsen/<?=$pegawai['username']?>?token=<?=$_GET['token']?>',{
		method:'POST',
		body:formData
	})
